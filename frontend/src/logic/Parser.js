import { INSTRUCTIONS, INSTRUCTION_INPUT } from "./Instructions";
...
const nameToInstruction = Object.fromEntries(
  Object.entries(INSTRUCTIONS).map((entry) => [entry[1].name, INSTRUCTIONS[entry[0]]])
);
...
const instructionFromTokens = (tokens) => {
  ...
  if (!instructionDefinition) throw new Error(`Cannot find instruction with name ${name}.`);
  ...
  const zipped = inputTokens.map((e, i) => [e, instructionDefinition.inputs[i]]);
...
const extractLabels = (instructions) => {
  ...
  const noLabels = withRandomIds.filter((inst) => inst.name !== INSTRUCTIONS.LABEL.name);
  const onlyLabels = withRandomIds.filter((inst) => inst.name === INSTRUCTIONS.LABEL.name);
  const labels = onlyLabels.map((labelInst) => {
    const name = labelInst.inputs[0].value.replace(":", "");
    const labelIndex = withRandomIds.findIndex((inst) => inst.id === labelInst.id);
    const nextNonLabelInstruction = findNextNonlabelInstruction(withRandomIds, labelIndex);
    const index = noLabels.findIndex((inst) => inst.id === nextNonLabelInstruction.id);
    return { name, index };
  });
  return labels;
};
